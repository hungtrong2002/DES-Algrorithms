    ...
    public class DesEncryption
    {
        ...
        public static string StringToBinary(string input)
        {
            ...
            foreach (char c in input)
            {
                ...
                // Chuyển mã ASCII thành chuỗi nhị phân 8 bit
        ...
        public static string BinaryToString(string binaryString)
        {
            ...
            // Duyệt qua chuỗi nhị phân với bước là 8 ký tự
            for (int i = 0; i < binaryString.Length; i += 8)
            {
                // Lấy 8 bit liên tiếp
                string eightBits = binaryString.Substring(i, 8);

                // Chuyển đổi từ nhị phân sang mã ASCII
                int asciiValue = Convert.ToInt32(eightBits, 2);

                // Chuyển mã ASCII thành ký tự và nối vào kết quả
        ...
        private static string preProcessText(string plain_text)
        {
            // Xử lý chuỗi nhị phân, thêm các bit 0 vào cuối nếu cần
            int remainder = plain_text.Length % 64;
            if (remainder != 0)
            {
                int paddingLength = 64 - remainder;
                plain_text += new string('0', paddingLength);
            }
            return plain_text;
        }
        private static string InputKeyJudge(string binaryKey)
        {
            // Kiểm tra độ dài của khóa, nếu nhỏ hơn 64 bit thì thêm bit 0 vào cuối
            if (binaryKey.Length < 64)
            {
                int paddingLength = 64 - binaryKey.Length;
                binaryKey += new string('0', paddingLength);
            }
            // Nếu lớn hơn 64 bit, cắt bớt
        ...
        public static string DecryptAllMessages(string cipher_text, string key)
        {
            // Chuyển đổi tin nhắn và khóa sang định dạng nhị phân
            string binaryMessage = StringToBinary(cipher_text);
            string binaryKey = StringToBinary(key);

            // Xử lý chuỗi nhị phân tin nhắn
            string processedMessage = preProcessText(binaryMessage);

            // Chuyển đổi khóa vào định dạng chính xác
            string processedKey = InputKeyJudge(binaryKey);
        ...
        {
            string result = "";
            List<string> blocks = new List<string>(Regex.Split(cipher_text, "(?<=\\G.{64})"));
            foreach (string block in blocks)
            {
                if (!string.IsNullOrWhiteSpace(block.Trim()))
                {
                    result += Decrypt(block, binaryKey); // Encrypt each block and append the result
                }
            }
            return result;
        }
        private static string DesEnscryptAll(string plain_text, string binaryKey)
        {
            string res = "";
            // Split message into blocks of 64 bits each
            string[] blocks = Regex.Split(plain_text, "(?<=\\G.{64})");
            ...
            foreach (string block in blocks)
            {
                if (!string.IsNullOrWhiteSpace(block.Trim()))
                {
                    res += Encrypt(block, binaryKey); // Encrypt each block and append the result
                }
            }
            return res;
        }
        public static string EncryptAllMessages(string plain_text, string key)
        {

            // Chuyển đổi tin nhắn và khóa sang định dạng nhị phân
            string binaryMessage = StringToBinary(plain_text);
            string binaryKey = StringToBinary(key);
            // Xử lý chuỗi nhị phân tin nhắn
            string processedMessage = preProcessText(binaryMessage);
            // Chuyển đổi khóa vào định dạng chính xác
        ...
        {
            // Initial Permutation
            string permuted_text = InitialPermutation(plain_text);
            ...
            // Final Permutation
            string cipher_text = FinalPermutation(right + left);
        ...
        public static string Decrypt(string cipher_text, string key)
        {
            ...
            // Initial Permutation
            string permuted_text = InitialPermutation(cipher_text);
